...
def get_recommendations(model, coo_mtrx, users_ids):
    ...
    for user in users_ids:
        ...
        for x in top_scores.tolist():
            for artist, values in data['artists'].items():  #itertems changed to items
                if int(x) == values['id']:
                    print ('   - %s' % values['name'])
...
user_1 = input('Select user_1 (0 to %s): ' % data['users']) #edited raw_input() to input() bc it was renamed to input()
